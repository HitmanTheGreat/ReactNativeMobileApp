...
const CropDetailsScreen = ({ route }) => {
    const { cropId } = route.params;  // Get cropId from the navigation params
    ...
    useEffect(() => {
        const fetchCropData = async () => {
            try {
                const response = await getRequest(`/crops/${cropId}/`, {}, token);
                setCropData(response);  // Assuming the crop data is inside response.data
            } catch (error) {
    ...
    const handleDelete = async () => {
        try {
            await deleteRequest(`/crops/${cropId}/`, {}, token);  // Send DELETE request
    ...
    const handleSave = async () => {
        try {
            ...
            Toast.show({
                type: 'success',
                text1: 'Success!',
                text2: 'Crop data has been saved successfully.',
            });
        } catch (error) {
            ...
            Toast.show({
                type: 'error',
                text1: 'Error!',
                text2: 'Failed to save crop data. Please try again.',
            });
        }
    };

    const handleChange = (key, value) => {
        setCropData(prevData => ({ ...prevData, [key]: value }));
    };
        ...
        <View style={styles.container}>
            <Animated.View entering={FadeInUp.duration(800)} style={styles.card}>
                ...
                {Object.keys(cropData)
                    .filter(key => !['id', 'createdAt', 'updatedAt'].includes(key)) // Skip these fields
                    .map((key, index) => (
                        <Animated.View key={key} entering={FadeInUp.delay(200 * index).duration(600)} style={styles.inputContainer}>
                            ...
                            {/* Conditionally render a dropdown for 'cropType' using RNPickerSelect */}
                            {key === 'cropType' ? (
                                isEditing ? (
                                    <RNPickerSelect
                                        style={pickerSelectStyles}
                                        value={cropData[key]} // The selected value of the dropdown
                                        onValueChange={(value) => handleChange('cropType', value)} // Updates the cropType value
                                        items={[
                                    ...
                                    <Text style={styles.text}>{cropData[key]}</Text>
                                )
                            ) : (
                                isEditing ? (
                                    <TextInput
                                        style={styles.input}
                                        value={cropData[key]}
                                        onChangeText={(text) => setCropData({ ...cropData, [key]: text })}
                                    />
                                ) : (
                                    <Text style={styles.text}>{cropData[key]}</Text>
                                )
            ...
            <Modal visible={isModalVisible} transparent animationType="fade">
                <View style={styles.modalContainer}>
                    <View style={styles.modalContent}>
                        <Text style={styles.modalText}>Are you sure you want to delete this crop?</Text>
